...
function createField() {
    for (const input of inputs){
        ...
        if (input === "age" || input === "salary") inputfield.type = "number";
...
button.addEventListener("click", (e)=>{
    ...
    for(const key in formValues){
        ...
        if (key === "salary") td.textContent = `$${Number(formValues[key]).toLocaleString("en-US")}`;
        else td.textContent = formValues[key];
        tr.append(td);
    }
...
function getCellValue(row, index) {
    ...
    if (headerText === "age" || headerText === "salary") return Number(value.replaceAll("$", "").replaceAll(",", ""));
    return value;
}
for(let i = 0; i < headers.length; i++){
    ...
    header.addEventListener("click", function() {
        ...
        rows.sort(function(rowA, rowB) {
            ...
            if (typeof valueA === "number") return valueA - valueB;
